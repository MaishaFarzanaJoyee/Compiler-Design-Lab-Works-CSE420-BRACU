...
vector<string> current_function_param_types; 
vector<string> current_function_param_names; 

string current_variable_type;        
vector<string> current_variable_names; 
...
func_definition : type_specifier ID LPAREN parameter_list RPAREN  enter_function compound_statement
		{	
		...
		| type_specifier ID LPAREN RPAREN {
			...
			symbol_info* func_sym = new symbol_info($2->get_name(), $1->get_name()); // or just name/type
			func_sym->set_type("Function Definition"); // set type
			func_sym->set_return_type($1->get_name());         // set return type

			symtbl->insert(func_sym); // insert into your symbol table
...
enter_function :
{
    ...
    // 2. Store function context
    current_function_name = $-2->get_name();       // function name
    current_function_return_type = $-3->get_name(); // return type

    // 3. Prepare a new symbol_info object for this function
    symbol_info* func_sym = new symbol_info(current_function_name, current_function_return_type);

    // 4. Set type as "Function Definition"
    func_sym->set_type("Function Definition");

    // 5. Add parameter types and names
    for(size_t i = 0; i < current_function_param_names.size(); i++)
    {
        string pname = current_function_param_names[i];
        string ptype = current_function_param_types[i];

       
         
        func_sym->add_parameter(ptype);

        // Insert parameter into current scope
        if(symtbl->lookup(new symbol_info(pname, ptype)) == nullptr) {
            symtbl->insert(new symbol_info(pname, ptype));
        } else {
            yyerror(("Parameter redeclaration: " + pname).c_str());
        }
    }

    
    if(symtbl->lookup(func_sym) == nullptr) {
        symtbl->insert(func_sym);
    } else {
        yyerror(("Function redeclaration: " + current_function_name).c_str());
    }
...
parameter_list : parameter_list COMMA type_specifier ID
		{
			...
			current_function_param_types.push_back($3->get_name());
			current_function_param_names.push_back($4->get_name());
		}
		| parameter_list COMMA type_specifier
		{
 		...
 		| type_specifier ID
 		{
            ...
            // store the necessary information about the function parameters
            // They will be needed when you want to enter the function into the symbol table
		    current_function_param_types.push_back($1->get_name());
    		current_function_param_names.push_back($2->get_name());
		}
		| type_specifier
		{
...
compound_statement : LCURL { symtbl->enter_scope(outlog); } statements RCURL
			{ 
...
var_declaration : type_specifier declaration_list SEMICOLON
		 {
			...
			symbol_info* sym = new symbol_info($2->get_name(), $1->get_name()); // name, type
if (!symtbl->insert(sym))
{
    outlog << "Error: Variable '" << $2->get_name() << "' redeclared ..." << endl;
}
...
declaration_list : declaration_list COMMA ID //int a, b 
		  {
